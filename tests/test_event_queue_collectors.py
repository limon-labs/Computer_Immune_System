...
def test_process_event_collector_publishes_started_event():
    ...
    collector = ProcessEventCollector(RealTimeProcessMonitor(FakeProcessMonitor(), poll_interval=0), event_queue)
...
def test_etw_collector_reports_unsupported_on_non_windows():
    ...
    if collector.supported():
        assert collector.start() in {"ready", "missing_pywin32_dependency"}
    else:
...
def test_etw_ingest_normalizes_event_payload():
    ...
    assert event_queue.drain(1)[0].payload["pid"] == 42
